"""
Combined engine + batch runner — glycogen 0.33 fixed, oxygen grid series
[0.3, 0.35, 0.4, 0.45, 0.5], scenario='none' only (no wound), threshold=500.
...
file as the old 300-run series (runs_series_2026-08-17.csv), because that
series has no oxyRev column and used the old default revStore=.2. Mixing
the two would make (scenario, seed) rows indistinguishable between two
different parameter regimes.

BEFORE running the full series: run the short validation at the bottom
of this file (VALIDATE = True) — 1 seed, scenario 'none', short horizon,
revStore=0.33 / oxyRev=0.4 — and send rev_total / rev_oxy_death /
final_density back for cross-check against the JS bench on the same seed.
Only flip to the full series (VALIDATE = False) after that checks out.
...
if __name__ == '__main__' and VALIDATE:
    ...
    print(f"Validation run: revStore=0.33, oxyRev=0.4, seed={VAL_SEED}, "
          f"{VAL_HOURS}h, scenario='none' (no wound)")
    t0 = time.time()
    row = run_one(
        run_id=0, params=dict(lifeD=6.0, revStore=0.33, oxyRev=0.4),
        seed=VAL_SEED, target_hours=VAL_HOURS, scenario='none',
    )
    dt = time.time() - t0
    ...
    print("\nSend these four numbers (rev_total, rev_oxy_death, "
...
#
# This series is an OXYGEN GRID at fixed glycogen (revStore=0.33),
# scenario='none' ONLY (no wound — this is about the spontaneous
# stability boundary, not injury-driven establishment), stop_threshold=500.
# ============================================================
if __name__ == '__main__':
    # 100 seeds per oxyRev value, grid = [0.3, 0.35, 0.4, 0.45, 0.5]
    # -> 500 runs total. Same seed set (0..N_SEEDS-1) reused across every
    # oxyRev value on purpose — paired design, same initial tissues per seed
    # across the whole grid, removes some between-point variance.
    TARGET_HOURS = 20000
    STOP_THRESHOLD = 500
    OXY_GRID = [0.3, 0.35, 0.4, 0.45, 0.5]
    N_SEEDS = 100
    SCENARIO = 'none'   # no wound for this series

    # NEW file — one series, one file. Every oxyRev value lives in this same
    # CSV distinguished by the oxyRev column; don't reuse this name for a
    # different grid or a different revStore.
    OUT_CSV = 'runs_series_oxygrid_2026-08-29.csv'   # keep the SAME name across sessions
    ...
    # Key is (oxyRev, seed) now, not (scenario, seed) — scenario is fixed to
    # 'none' for this whole series, but oxyRev varies, so it has to be part
    # of the resume key or two different oxyRev values would collide.
    done = set()
    if os.path.exists(OUT_CSV):
        with open(OUT_CSV, newline='') as fh:
            for r in csv.DictReader(fh):
                done.add((round(float(r['oxyRev']), 6), int(r['seed'])))
        print(f"found {len(done)} completed runs in {OUT_CSV} — skipping those")

    # run_id derived from (oxyRev index in the grid, seed) so it stays stable
    # if a later session runs only a subset of the grid. The real key for
    # joining in pandas is still (oxyRev, seed), not run_id.
    jobs = []
    for oxy_idx, oxy in enumerate(OXY_GRID):
        for seed in range(N_SEEDS):
            key = (round(oxy, 6), seed)
            if key in done:
                continue
            run_id = oxy_idx * 100000 + seed
            jobs.append(dict(
                run_id=run_id,
