"""
Combined engine + batch runner — glycogen 0.33 fixed, wound-grid series:
oxyRev in [0.35, 0.4, 0.45, 0.5] x scenario in ['bounded', 'chronic'],
threshold=500. Complements the earlier scenario='none' oxygen-grid series
(runs_series_oxygrid_2026-08-29.csv) — that file already covers the
no-wound point at oxyRev in [0.3, 0.35, 0.4, 0.45, 0.5], so it is NOT
repeated here.
...
(oxygen filter) mechanism added to the engine:
  ...
  - oxyRev: probability that a newly-created reverted-lineage daughter
    ...
    mechanism exactly, same RNG order). Default 1.0 (no filter) so any
    OLD code that doesn't pass oxyRev still behaves as before.
  - new counter rev_oxy_death: how many would-be reverted cells died at
    birth because they failed the O2 roll.

Wound protocol (bounded / chronic) matches the Chapter-1 historical run
for comparability: wound_start_h=1000, wound_size=200, wound_freq_h drawn
per-run from {12, 24, 48}h. 'bounded' stops wounding once the clone first
crosses stop_threshold; 'chronic' keeps wounding to the end of the horizon
regardless of clone size.

Output goes to a NEW csv (see OUT_CSV below) — deliberately NOT the same
file as either the old 300-run series (runs_series_2026-08-17.csv, no
oxyRev column, revStore=1.0) or the no-wound oxygrid series
(runs_series_oxygrid_2026-08-29.csv, scenario='none' only). Mixing any of
these would make (scenario, oxyRev, seed) rows indistinguishable between
...
final_density / wounds_inflicted back for cross-check against the JS bench
on the same seed. Only flip to the full series (VALIDATE = False) after
that checks out.
...
#
# This series is a WOUND GRID at fixed glycogen (revStore=0.33):
# oxyRev in [0.35, 0.4, 0.45, 0.5] x scenario in ['bounded', 'chronic'].
# The scenario='none' point at this same revStore already exists in
# runs_series_oxygrid_2026-08-29.csv and is intentionally NOT rerun here.
# ============================================================
if __name__ == '__main__':
    # 100 seeds per (scenario, oxyRev) combination, grid = [0.35, 0.4, 0.45, 0.5]
    # x scenarios = ['bounded', 'chronic'] -> 800 runs total. Same seed set
    # (0..N_SEEDS-1) reused across every combination on purpose — paired
    # design, same initial tissues per seed across the whole grid.
    TARGET_HOURS = 20000
    STOP_THRESHOLD = 500
    OXY_GRID = [0.35, 0.4, 0.45, 0.5]
    SCENARIOS = ['bounded', 'chronic']
    N_SEEDS = 100

    # Wound protocol — matches the Chapter-1 historical run for
    # comparability. wound_freq_h is randomized per run (not fixed for the
    # whole series), same as it was in that earlier run.
    WOUND_START_H = 1000
    WOUND_SIZE = 200
    WOUND_FREQ_CHOICES = [12, 24, 48]
    FREQ_DRAW_SEED = 12345   # fixed, so the freq draw is reproducible

    # NEW file — do not reuse either the old 300-run series name or the
    # no-wound oxygrid name; both use different parameters/columns.
    OUT_CSV = 'runs_series_oxygrid_wound_2026-09.csv'
    ...
    # Key is (scenario, oxyRev, seed) now — scenario AND oxyRev both vary
    # in this series, so both have to be part of the resume key or rows
    # from different combinations would collide.
    done = set()
    if os.path.exists(OUT_CSV):
        with open(OUT_CSV, newline='') as fh:
            for r in csv.DictReader(fh):
                done.add((r['scenario'], round(float(r['oxyRev']), 6), int(r['seed'])))
        print(f"found {len(done)} completed runs in {OUT_CSV} — skipping those")

    # run_id derived from (scenario index, oxyRev index, seed) so it stays
    # stable if a later session runs only a subset of the grid. The real
    # key for joining in pandas is still (scenario, oxyRev, seed), not run_id.
    jobs = []
    freq_rng = random.Random(FREQ_DRAW_SEED)
    for scen_idx, scenario in enumerate(SCENARIOS):
        for oxy_idx, oxy in enumerate(OXY_GRID):
            for seed in range(N_SEEDS):
                key = (scenario, round(oxy, 6), seed)
                # Draw the freq regardless of whether we skip, so the
                # sequence of draws is identical across resumed sessions.
                wound_freq_h = freq_rng.choice(WOUND_FREQ_CHOICES)
                if key in done:
                    continue
                run_id = scen_idx * 1000000 + oxy_idx * 100000 + seed
                jobs.append(dict(
                    run_id=run_id,
                    params=dict(lifeD=6.0, revStore=0.33, oxyRev=oxy),
                    seed=seed,
    ...
    total = len(SCENARIOS) * len(OXY_GRID) * N_SEEDS
