"""
Combined engine + batch runner — GLYCOGEN GRID, scenario='none' ONLY,
oxygen filter OFF (oxyRev=1.0): revStore in [0.20, 0.25, 0.30],
threshold=500. This is the postponed no-wound complement to
runs_series_glycogen_grid_wound_2026-09.csv (which covers bounded/chronic
at these same revStore values) — fills in the 'none' row so these three
glycogen values have full three-scenario coverage, matching revStore=0.33
(runs_series_baseline_033_2026-09.csv, all three scenarios already done).
...
if __name__ == '__main__':
    # 100 seeds per revStore value, grid = [0.20, 0.25, 0.30], scenario='none'
    # ONLY -> 300 runs total. This is the deliberately-postponed no-wound
    # complement to the bounded/chronic glycogen grid
    # (runs_series_glycogen_grid_wound_2026-09.csv) — fills in the 'none'
    # row for these three revStore values, matching how 0.33 already has
    # all three scenarios covered.
    TARGET_HOURS = 20000
    STOP_THRESHOLD = 500
    OXY_REV = 1.0          # filter off — same no-filter condition as every other series here
    REV_STORE_GRID = [0.20, 0.25, 0.30]
    SCENARIO = 'none'
    N_SEEDS = 100

    # NEW file — 'none' has no wound columns to speak of, so it gets its
    # own file rather than being mixed into the bounded/chronic one.
    OUT_CSV = 'runs_series_glycogen_grid_none_2026-09.csv'
    ...
    # Key is (revStore, seed) — scenario is fixed to 'none' for this whole
    # series, so only revStore and seed can distinguish rows.
    done = set()
    if os.path.exists(OUT_CSV):
        with open(OUT_CSV, newline='') as fh:
            for r in csv.DictReader(fh):
                done.add((round(float(r['revStore']), 6), int(r['seed'])))
        print(f"found {len(done)} completed runs in {OUT_CSV} — skipping those")

    # run_id derived from (revStore index, seed) so it stays stable if a
    # later session runs only a subset. The real key for joining in pandas
    # is (revStore, seed), not run_id.
    jobs = []
    for rs_idx, rev_store in enumerate(REV_STORE_GRID):
        for seed in range(N_SEEDS):
            key = (round(rev_store, 6), seed)
            if key in done:
                continue
            run_id = rs_idx * 1000000 + seed
            jobs.append(dict(
                run_id=run_id,
    ...
    total = len(REV_STORE_GRID) * N_SEEDS
