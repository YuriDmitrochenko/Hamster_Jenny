"""
Combined engine + batch runner — revStore=0.33/oxyRev=1.0 BASELINE series,
all three scenarios: 'none', 'bounded', 'chronic', threshold=500. This is
the correct no-filter comparison point for chapters 1-2 (they currently
compare against an older run that used FULL, untruncated glycogen instead
of revStore=0.33 — this series replaces that comparison point).
...
if __name__ == '__main__':
    # 100 seeds per scenario, scenarios = ['none', 'bounded', 'chronic']
    # -> 300 runs total. Same seed set (0..N_SEEDS-1) reused across every
    # scenario on purpose — paired design, same initial tissues per seed.
    TARGET_HOURS = 20000
    STOP_THRESHOLD = 500
    OXY_REV = 1.0          # filter off — this whole series is the no-filter baseline
    REV_STORE = 0.33
    SCENARIOS = ['none', 'bounded', 'chronic']
    N_SEEDS = 100

    # Wound protocol for 'bounded'/'chronic' — matches every other series in
    # this file for comparability. Ignored entirely for 'none' (run_one
    # never activates wounding unless scenario is 'bounded' or 'chronic').
    WOUND_START_H = 1000
    WOUND_SIZE = 200
    WOUND_FREQ_CHOICES = [12, 24, 48]
    FREQ_DRAW_SEED = 12345   # fixed, so the freq draw is reproducible

    # NEW file — don't reuse any earlier series name; this one has a fixed
    # oxyRev (no oxyRev column varying) and includes 'none', which the
    # earlier wound-grid file deliberately excluded.
    OUT_CSV = 'runs_series_baseline_033_2026-09.csv'
    ...
    # Key is (scenario, seed) — oxyRev and revStore are both fixed for the
    # whole series, so only scenario and seed can distinguish rows.
    done = set()
    ...
    # run_id derived from (scenario index, seed) so it stays stable if a
    # later session runs only a subset. The real key for joining in pandas
    # is still (scenario, seed), not run_id.
    jobs = []
    freq_rng = random.Random(FREQ_DRAW_SEED)
    for scen_idx, scenario in enumerate(SCENARIOS):
        for seed in range(N_SEEDS):
            key = (scenario, seed)
            # Draw the freq regardless of whether we skip or whether this
            # scenario even uses it, so the draw sequence is identical
            # across resumed sessions and across scenarios.
            wound_freq_h = freq_rng.choice(WOUND_FREQ_CHOICES)
            if key in done:
                continue
            run_id = scen_idx * 1000000 + seed
            if scenario == 'none':
                job = dict(
                    run_id=run_id,
